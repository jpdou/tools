...
class Fishdrowned_Layoutviewer_Model_Observer extends Varien_Object
{
     ...
     */
    public function blockToHtmlBeginTime($observer)
    {
        $block = $observer->getData('block');
        S::getExpendedTime($this->_time);
        S::performanceAnalyseLog(get_class($block). '['. $this->_time['current']. ' '. $this->_time['increment']. ']');
    }

    /**
     * @param Varien_Event $observer
     */
    public function blockToHtmlEndTime($observer)
    {
        $block = $observer->getData('block');
        S::getExpendedTime($this->_time);
        S::performanceAnalyseLog(get_class($block). '['. $this->_time['current']. ' '. $this->_time['increment']. ']');
